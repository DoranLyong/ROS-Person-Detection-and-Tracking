...
from RosImage import ImageListener
...
class PersonDetector(object):
    ...
    def __init__(self, topic):
        ...
        rospy.Subscriber(self.topic , Image , self.imageCallback)
    ...
    def imageCallback(self, img_msg):
        ...
        try:
            cv2_img  = self.bridge.imgmsg_to_cv2(img_msg, desired_encoding='bgr8')   # Cvt ROS image msg to cv2 img 
    ...
    def __call__(self, yolo_module, reid_model):
        ...
        print("images: ", cv2_img )

#        cv2.imshow("inside", self.cv2_img)
        

#        while True: 
#            frame = cv2.cvtColor(self.cv2_img, cv2.COLOR_BGR2RGB)
...
def emergency_detection(percep_req):
    ...
    imgs, bboxes = PersonDetector(topic=camera_topic)(yolo_module, reid_model) # class init. and call 
